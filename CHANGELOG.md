...
- Interactable controls cast light-driven cut shadows: the single best-lit control over a surface (the hovered one always wins) throws one large hard-edged wedge shadow, projected ray-style from the pointer — hovering a button sweeps a long shadow away from the mouse and flares it open across the surface; with the mouse away the shadow rests on a key light at the surface's right edge. Darkness scales with how lit the control is. data-bc-caster tags any element as a caster.
