...
- Interactable controls cast hover-driven wedge shadows: the hovered control over a WebGL surface throws one large shadow along the fixed top-right to bottom-left key direction — a pure wedge whose exactly two straight hard edge lines start at the control's bottom corners (the grazing light rays), masked flush above the control, eased in and out so it never snaps; wider for bigger controls. data-bc-caster tags any element as a caster.
