...
- WebGL effects now paint in the surface's own colour: each element's background, lifted toward light, is the silk's base — blended toward the tint colour in proportion to the tint strength — so the effect reads as sheen on the component instead of a coloured wash, and stays subtle at low strengths. Colours re-resolve on theme flips.
