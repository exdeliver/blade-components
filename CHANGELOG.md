...
- Interactable controls cast hover-driven wedge shadows: the hovered control over a WebGL surface throws one large shadow along the fixed top-right to bottom-left key direction — a pure wedge with its apex at the control's corner and exactly two straight hard edges, masked flush above the control, eased in and out so it never snaps; wider for bigger controls. data-bc-caster tags any element as a caster.
