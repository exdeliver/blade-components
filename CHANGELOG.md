...
### Added
- Badge/Lozenge `outline` prop: the variant is carried as an outer hairline (transparent fill, tinted text and border) instead of a fill — the quiet badge for dense lists like activity feeds.
- `--input` is now a real well token: the theme's field fill is mixed darker than both the canvas and the lifted surface (light and dark), so inputs and textareas read as pressed into their panel; the new `.bc-field` utility pairs it with an inset top shadow.
...
- WebGL surfaces survive Vue hydration: SSR-rendered surfaces (the login card) had their effect canvas stripped by hydration's in-place patch; the frame loop now re-attaches detached canvases and drops dead elements.
- Interactable controls cast hover-driven wedge shadows: the hovered control over a WebGL surface throws one large shadow along the fixed top-right to bottom-left key direction — a pure wedge whose exactly two straight hard edge lines start at the control's bottom corners (the grazing light rays), cut flush at the control's bottom line, eased in and out so it never snaps; wider for bigger controls. data-bc-caster tags any element as a caster.
- WebGL surfaces cast light and shadow on each other: nearby tagged siblings become occluders in the shader and every light (drifting blobs and the pointer) projects their silhouettes as shadows — hard-edged when close, softer with distance. The render loop is layout-free (rects cached from resize/scroll/pointer events) and the master canvas no longer uses a preserved drawing buffer.
- WebGL effects are now a coherent ambient light rig — slow drifting light blobs, a soft edge bleed and the pointer light, all on one shared distance falloff — painting the element's own surface colour (its background lifted toward light, blended toward the tint only in proportion to tint strength). Reads as real lighting on the component rather than a coloured overlay; colours re-resolve on theme flips.
- Single shared WebGL context for the surface effects: all registered surfaces render into one offscreen atlas and each element's canvas is a 2D blit of its slice. The per-element-context version exceeded the browser's live-context cap on pages with many surfaces (evicted contexts composite as white on some drivers); 2D canvases have no such cap, and the shared context releases all surfaces to their CSS fallback if the driver resets it.
