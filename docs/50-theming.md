...
Per surface, `data-bc-gain` dials the whole rig — lift and cast shadow together — toward the flat tint: `0.5` barely whispers, `1` is the full rig, and untagged surfaces get the subtle `0.72` default, so each page can set its own brightness. Form fields take the `.bc-field` class — the theme's darker `--input` well fill plus an inset top shadow — so they read as pressed into whatever panel holds them; the theme's `--input` token itself now sits below both the canvas and the lifted surface in either appearance. Content panels that must stay legible under a neighbour's cast shadow (tab bodies, detail panes) take the `bc-depth` class: it paints a semi-opaque tint veil over the canvas behind them and adds an inset hairline, so the panel reads as its own recessed depth step and the shadow only grazes its edge.
